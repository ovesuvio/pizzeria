...
export default async function handler(req, res) {
  ...
  if (files.length > 20) return res.status(400).json({ error: 'Troppi file' });
  ...
  for (const f of files) {
    ...
    if (buf.length > 10 * 1024 * 1024) return res.status(400).json({ error: 'File troppo grande' });
    fs.writeFileSync(target, buf);
    saved.push({ file: `/gallery/${index}${ext}` });
    index += 1;
  }
